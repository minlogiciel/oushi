...
include ($DOC_PATH."/album/album_items.inc");
$curr_url = $_SERVER["PHP_SELF"]; 

$n_next = 1;
$n_prev = -1;
if ($albumindex < 0) {
	$atitle = $ALBUM_ITEM_NAME[0][2];
	$aimage = $ALBUM_ITEM_NAME[0][6];
	$n_album = 0;
	for ($i = 1; $i < count($ALBUM_ITEM_NAME); $i++) {
		$active = $ALBUM_ITEM_NAME[$i][7];
		if ($active) {
			$atitle = $ALBUM_ITEM_NAME[$i][2];
	...
	$atitle = $ALBUM_ITEM_NAME[$albumindex][2];
...
<?php 
	...
	echo("&nbsp;&nbsp;<a href='".$HOST_URL."/french/album.php?groups=".$n_album."'>".$atitle."</a>&nbsp;&nbsp;"); 
    if ($n_next > 0) {
...
?>
    ...
    <div class="img">
<?php 
		echo("<a href='".$HOST_URL."/french/album.php?groups=".$n_album."'><IMG src='".$PHOTO_PATH."/album/".$aimage."' border='0' width='280px'></a>");
?>
    </div>
</div>
