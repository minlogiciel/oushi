...
function addNewsPage($news_tab, $news_type) {
	global $ADDINFOITEMS;
	...
	$filename = "../" .$ADDINFOITEMS[$news_type][1]. "/".strtolower($varname).".inc";
	$fp = fopen($filename, "w");
	fwrite($fp, $text);
	fclose($fp);
...
function showNewsNewItemForm($stitle, $imgfile, $news, $i, $f) {
	$TT = array("", "中文", "法文");
...
<TR>
	...
	<TD class=labelleft>
		<INPUT class=fields type=text size=63 name="<?php echo("imgfile_".$i."_".$f); ?>" value="<?php echo($imgfile); ?>">
	</TD>
</TR>
<TR>
...
<TR>
	...
	<TD class=labelleft>
		<INPUT class=fields type=text size=63 name="<?php echo("other_".$i."_".$f."_1"); ?>" value=""> 
	</TD>
</TR>
</TABLE>
<?php
...
function showNewsItemForm($item, $i, $f) {
	$stitle 	= "";
	$news 		= "";
	$imgfile	= "";
	$TT = array("", "中文", "法文");
...
<TABLE cellSpacing=0 cellPadding=0 width=100% border=0 align=center>
<TR>
	<TD width=30% class=labelright1 height=30><?php echo($TT[$f]); ?>副标题 : </TD>
	<TD width=70% class=labelleft>
		<INPUT class=fields type=text size=40 name="<?php echo("subtitle_".$i."_".$f); ?>" value="<?php echo($stitle); ?>">
...
?>
		<TR>
			<TD width=100% class=labelleft height=30>
				...
				<INPUT class=fields type=text size=50 name="<?php echo("weekday_str_".$i."_".$f."_".$t); ?>" value="<?php echo($tstr); ?>"> 
			</TD>
		</TR>
<?php } ?>
		</TABLE>
...
<TR>
	...
	<TD class=labelleft>
		<INPUT class=fields type=text size=50 name="<?php echo("other_".$i."_".$f."_".$p); ?>" value="<?php echo($other[$p][1]); ?>"> 
	</TD>
</TR>
<?php } } ?>
</TABLE>
...
function showNewsPageForm($jlnews, $mindex, $nindex, $news_tab, $news_type, $msg) {
	global $ADDINFOITEMS;
	...
	if ($jlnews) {
		...
		if (count($v) > 3) {
			...
			$vname = "DIVERS_PAGE_".$mindex."_".$nindex;
		}
...
<INPUT type=hidden name='action' value='addnewspage'>
...
<TR>
	...
	<?php 
	if ($isupdate) {
		echo("修改" .$ADDINFOITEMS[$news_type][0]."页面"); 
	} else { 
		echo("添加" .$ADDINFOITEMS[$news_type][0]."页面"); 
	} ?>
	</h2>
	</TD>
</TR>
	...
	<TD>
		...
		<TR>
			<TD >
				<TABLE cellSpacing=0 cellPadding=0 width=98% border=0 align=center>
				<TR>
					<TD width=30% class=labelright1 height=30>中文题目 : </TD>
					<TD width=70% class=labelleft>
						<INPUT class=fields type=text size=40 name="title" value="<?php echo($title); ?>">
					</TD>
				</TR>
		<?php if ($item) { ?>
				<TR><TD colspan=2><?php $this->showNewsItemForm($item, 3, 1); ?></TD></TR>
				<TR><TD colspan=2 height=15></TD></TR>
			<?php for ($i = 4; $i < $nb_item; $i++ ) { $sitem = $item[$i]; ?>
				<TR><TD colspan=2><?php $this->showNewsItemForm($sitem, $i, 1); ?></TD></TR>
				<TR><TD colspan=2 height=15></TD></TR>
			<?php  	} 	
			} else { ?>
				<TR>
					<TD colspan=2>
					<?php $this->showNewsNewItemForm($stitle, $imgfile, $news, 3, 1);  ?>
					</TD>
				</TR>
			<?php } ?>
				<TR><TD colspan=2 height=15></TD></TR>		
				<TR>
					...
					<TD class=labelleft>
						<INPUT class=fields type=text size=40 name="ftitle" value="<?php echo($ftitle); ?>">
					</TD>
				</TR>
	<?php  if ($fitem) { ?>
				<TR><TD colspan=2><?php $this->showNewsItemForm($fitem, 3, 2); ?></TD></TR>
				<TR><TD colspan=2 height=15></TD></TR>

			<?php for ($i = 4; $i < $nb_item; $i++ ) { $sitem = $fitem[$i]; ?>
				<TR><TD colspan=2><?php $this->showNewsItemForm($sitem, $i, 2); ?></TD></TR>
				<TR><TD colspan=2 height=15></TD></TR>
			<?php  	} 
		}	else { 
			$i = 3;
			do { ?> 
				<TR>
					<TD colspan=2>
					<?php $this->showNewsNewItemForm($fstitle, $fimgfile, $fnews, $i, 2);  ?>
					</TD>
				</TR>
			<?php $i++;
			} while ($i < $nb_item) ;
